...
## Настройки приложения (веб-интерфейс)

Раздел **Настройки** (`/settings`) позволяет управлять:

| Секция | Что настраивается |
|--------|------------------|
| **Webhooks** | Уведомления при ошибках бэкапа или недоступности устройства |
| **Планировщик** | Cron-расписание резервного копирования по умолчанию |
| **LDAP** | Аутентификация через Active Directory / LDAP |

### Изменение расписания резервного копирования

В разделе **Планировщик → Расписание по умолчанию** введите 5-польное cron-выражение:
...
После сохранения планировщик немедленно применяет новое расписание (без перезапуска сервиса). Значение сохраняется в SQLite и переживает перезапуск.

> **Приоритет расписания:** настройка из веб-интерфейса → `config.yaml (scheduler.default_cron)` → встроенное значение `0 3 * * *`.

Устройствам можно задать **индивидуальное** cron-расписание через форму редактирования устройства (`custom_cron`). Такие устройства не попадают под глобальное расписание.
